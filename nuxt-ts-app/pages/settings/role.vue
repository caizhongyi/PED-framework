<template>
    <div >
        ...
        <Modal :width="500" :title="`【${role}】权限分配`" v-model="showRootData"  @on-ok="submit" @on-cancel="fail">
...
<script lang="ts">
  ...
  export default class Role extends Vue {    //  typescript 创建类继成 Vue
    ...
    formModel = [
      { 　field : 'role' , type : 'input',  label: "角色名称", required: true },
      {   field : "description", label: "角色描述", type: "textarea", autosize: { minRows: 2, maxRows: 5 }, placeholder: "描述" },
    ]
        ...
        render: (h, params) => {
          ...
          return h('div', [
            ...
            h('Button', {
              ...
              on: {
                click: (e) => {
                  console.log('this',this.showRootData,params.row.role)
    ...
    //权限分配
    rootAssign(data){
      // console.log(1,data.row.role);
      this.role = data.row.role;
      // console.log(2,data.row.role);
      this.showRootData = true;
    }
