<template>
  <Dropdown @on-click="handleClick" transfer :placement="placement">
    <a class="drop-menu-a" type="text"  @mouseover="handleMousemove($event, children)" :style="{textAlign: !hideTitle ? 'left' : ''}">
      <Icon size="large"  :type="parentItem.icon"/><span class="menu-title" v-if="!hideTitle">{{ showTitle(parentItem) }}</span>
    ...
    <DropdownMenu slot="list">
      <template v-for="child in children"  v-if="child.isshow != false">
        ...
        <DropdownItem v-else :key="`drop-${child.name}`" :name="getNameOrHref(child , false ,name)"><Icon size="large" :type="child.icon"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>
