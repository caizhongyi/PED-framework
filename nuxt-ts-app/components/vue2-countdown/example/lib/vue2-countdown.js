var countdown = {
    template: '<div>' +
    '<p v-if="msTime.show">' +
    '<span v-if="tipShow&&tipVisible">{{tipText}}:</span>' +
    '<span v-if="!tipShow&&tipVisible">{{tipTextEnd}}:</span>' +
    '<span>{{msTime.day}}</span>{{ dayTxt }}' +
    '<span>{{msTime.hour}}</span>{{ hourTxt }}' +
    '<span>{{msTime.minutes}}</span>{{ minutesTxt }}' +
    '<span>{{msTime.seconds}}</span>{{ secondsTxt }}</p>' +
    ...
    methods: {
        init( times ){
            ...
            if(!times){
                ...
                this.end = times.end;
                this.current = times.current;
                this.star = times.star;
            }
